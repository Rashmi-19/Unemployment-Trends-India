...
db = client["mydb"]
UnemploymentCollection = db["unemployment_analysis"]
...
for i in range(len(stateNames)):
    results = UnemploymentCollection.find({"Region":stateNames[i]})
    results1 = UnemploymentCollection.find({"Region":stateNames[i]})
...
def plot_items_statewise(state):
    ...
    ax = sns.barplot(x=dates, y=unemployment)
...
def displayStates():
    ...
    global bg
    # Define image
    bg = ImageTk.PhotoImage(file="C:/Users/HP/Desktop/unemployment_pic1.png")
    ...
    back = Button(top, text="<<", padx=10, pady=8, command=top.destroy).grid(row=0, column=0, sticky=SE)
...
def plot_items_statewiseEmp(state):
    ...
    ax = sns.barplot(x=dates1, y=employment)
    ax.set_xlabel('Date of observation')
...
def plot_items_statewiseEstLabour(state):
    for i in range(len(state)):
        dates2.append(state[i][' Date']) # x-axis for the graph of state-wise analysis
...
def displayStates2():
    ...
    back = Button(top2, text="<<", padx=10, pady=8, command=top2.destroy).grid(row=0, column=0, sticky=SE)
...
# Define image
background = ImageTk.PhotoImage(file="C:/Users/HP/Desktop/unemployment_pic1.png")
